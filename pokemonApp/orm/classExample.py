...
class dummyPokemonClass(object):
	...
	def __init__(self, name, typeArrayStr, hp=100):
		#  It must be specified whether it is a primary key or not, the other elements can be ignored/undefined.
		self.id = [str(uuid.uuid4()), "VARCHAR(255)", {"PRIMARY KEY": True, "AUTOINCREMENT": False, "NOT NULL": True, "FOREIGN KEY {} REFERENCES ...etc": False}]
...
if __name__ == "__main__":
	...
	'''
	dbSession = orm()
	pokSQL = dbSession.filterBy.get(tablename="dummyPokemonClass", name="Pikachu")[0]
	pika = dummyPokemonClass(*pokSQL)

	pokSQL = dbSession.filterBy.get(tablename="dummyPokemonClass", name="Crocordil")[0]
